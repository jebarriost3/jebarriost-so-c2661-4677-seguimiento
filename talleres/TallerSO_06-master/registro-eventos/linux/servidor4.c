...
main(int argc, char *argv[]) {
  ...
  syslog(LOG_INFO, "Servidor iniciado. peticion=%s solicitud=%s",
         tuberia_peticion, tuberia_solicitud);
  ...
  for (;;) {
    ...
    if (df_lectura == -2 &&
      ...
      syslog(LOG_ERR, "Error abriendo %s: %s",
             tuberia_peticion, strerror(errno));
      closelog();
      _exit(EXIT_FAILURE);
    }
    ...
    if (caracteres_leidos < 0) {
      syslog(LOG_ERR, "Error leyendo peticiones: %s", strerror(errno));
      close(df_lectura);
      break;
    }
    ...
    if (caracteres_leidos > 0) {
      ...
      syslog(LOG_INFO, "Cliente %d conectado, peticion=%d",
      ...
      if ((df_escritura = open(tuberia_solicitud, O_WRONLY)) == -1) {
        syslog(LOG_ERR, "Error abriendo %s: %s",
               tuberia_solicitud, strerror(errno));
        closelog();
        _exit(EXIT_FAILURE);
      }
...
manejar_sigquit(int sig) {
  (void)sig;
  openlog("servidor4", LOG_CONS | LOG_PID, LOG_USER);
  syslog(LOG_INFO, "Servidor terminado por SIGQUIT");
